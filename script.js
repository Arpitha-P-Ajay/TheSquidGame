...
class RedLightGreenLightGame {
  ...
  playerCaught() {
    ...
    if (newLives <= 0) {
      ...
      this.updateLightDisplay();
      this.updateStatus(
        `🏁 Game Over! Final Score: ${this.gameStats.score} points in ${this.gameStats.round} rounds!`
      );
      ...
      this.updateLightDisplay();
      this.updateStatus(
        `💥 Caught moving! ${newLives} lives left - Get ready for next round!`
      );
      ...
      setTimeout(() => {
        if (this.gameState === "caught") {
          // Make sure we're still in caught state
    ...
    switch (this.gameState) {
      ...
      case "gameOver":
        ...
        lightElement.textContent =
          this.gameStats.lives <= 0 ? "GAME OVER" : "CAUGHT!";
        break;
      case "countdown":
    ...
    try {
      ...
      if (this.useFallback || !this.model) {
        ...
        try {
          ...
          const movingPrediction = prediction.find(
            (p) =>
          ...
          if (!movingPrediction) {
            const stillPrediction = prediction.find(
              (p) =>
                p.className.toLowerCase().includes("still") ||
                p.className.toLowerCase().includes("static") ||
                p.className === "Class 0"
            );
            movingProb = stillPrediction
              ? 1 - stillPrediction.probability
      ...
      if (this.gameState === "red" && movingProb > MOVE_DETECTION_THRESHOLD) {
        console.log(
          `Movement detected during RED LIGHT! ${(movingProb * 100).toFixed(
            1
          )}% > ${MOVE_DETECTION_THRESHOLD * 100}%`
        );
        this.playerCaught();
  ...
  updateDebugInfo() {
    ...
    this.predictions.forEach((prediction) => {
      ...
      debugItem.textContent = `${prediction.className}: ${(
